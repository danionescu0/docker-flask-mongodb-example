...
class MongoDb:
    ...
    def get(self, query: dict, limit: int = 10, offset: int = 0):
        return list(self.__connection[self.__collection].find(query).limit(limit).skip(offset))

    def upsert(self, key, data: dict):
        self.__connection[self.__collection].update_one({'_id': key}, {'$set': data}, upsert=True)
